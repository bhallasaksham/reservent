...
async def reserve_room(request: Request):
    try:
        createEventResponse = await facade(url="http://127.0.0.1:8080/events/create", http_verb='POST', request=request)
        if createEventResponse is not None:
            roomReservationResponse = await facade(url="http://127.0.0.1:8000/rooms/reserve", http_verb='POST',  request=request)
            if roomReservationResponse is not None:
                return await facade(url="http://127.0.0.1:8080/events/finalize", http_verb='PUT',  request=request);
    except HTTPException as e:
        return {"message": e.detail}
