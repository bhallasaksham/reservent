...
async def get_available_rooms(request: Request):
    try:
        return await facade(url='http://127.0.0.1:8000/rooms/available', http_verb='GET', headers=request.headers,
...
async def reserve_room(request: Request):
    ...
    try:
        ...
        if event.body is not None:
            roomReservationResponse = await facade(url="http://127.0.0.1:8000/rooms/reserve", http_verb='POST',
                                                   headers=request.headers, body=event.body)
            if roomReservationResponse is not None:
                return await facade(url="http://127.0.0.1:8080/events/finalize", http_verb='PUT',
                                    headers=request.headers, body=event.body)
    except HTTPException as e:
        return {"message": e.detail}
